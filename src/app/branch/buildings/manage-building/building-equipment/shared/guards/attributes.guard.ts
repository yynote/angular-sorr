...
export class AttributesGuard implements CanActivate {
  ...
  getDataFromApiOrStore(): Observable<any> {
    return this.store.pipe(
      select(fromStore.getAttributes),
      map(data => !!data),
      switchMap(loaded => {
        if (loaded) {
          return of(loaded);
  ...
  private getDataFromApi() {
    return this.equipmentService.getAllEquipmentAttributes(null)
      .pipe(
        tap(attributes => this.store.dispatch(new actions.SetEquipmentAttributes(attributes))),
        map(() => true)
      );
  }
}
